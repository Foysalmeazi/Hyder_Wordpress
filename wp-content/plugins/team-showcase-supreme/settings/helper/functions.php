...
if (!function_exists('wpm_check_license')) {
	function wpm_check_license($key, $autoUpdate = false)
	{
		...
		if (!$res) {
			...
			if(!$db_key){
				$wpdb->query("INSERT INTO {$wpdb->prefix}options(option_name, option_value) VALUES ('wpm_6310_license_key', '".$key."')");
			}else if($db_key != $key){
				$wpdb->query("UPDATE {$wpdb->prefix}options set option_value = '".$key."' where option_name = 'wpm_6310_license_key'");
			}
			if(!$autoUpdate){
